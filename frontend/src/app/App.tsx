    ...
    <>
      <div style={{ flex: 1, position: 'relative', overflow: 'hidden' }}>
        <div style={{ height: '100%', overflowY: 'auto', background: contentBg, transition: 'background 0.3s ease' }}>
          <Routes>
            <Route path="/"         element={<Home onOpenDeadline={() => setShowDeadlineModal(true)} />} />
            <Route path="/calendar" element={<CalendarView />} />
            <Route path="/calendar/:date" element={<DayDetail />} />
            <Route path="/report"   element={<WeeklyReport />} />
            <Route path="/profile"  element={<MyPage onLogout={onLogout} />} />
          </Routes>
        </div>
        <DeadlineModal
          open={showDeadlineModal}
          onClose={() => setShowDeadlineModal(false)}
          todo={{ text: '포폴 README 작성', deadline: '오후 9:00' }}
        />
      </div>
